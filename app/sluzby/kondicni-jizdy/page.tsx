    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <FadeIn>
            <div className="flex items-start gap-4 max-w-3xl">
              <div className="w-1 self-stretch min-h-[48px] bg-red shrink-0" />
      ...
      <section className="py-20 bg-graphite">
        <div className="max-w-7xl mx-auto px-6 space-y-14">
          <FadeIn>
            <div>
              <h2 className="font-display font-bold text-cream text-3xl uppercase tracking-wide mb-1">Motocykly A2 · A</h2>
              <div className="w-10 h-0.5 bg-red mb-8" />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {conditioning.motorcycle.map((item) => <PriceCard key={item.label + item.duration} item={item} />)}
              </div>
            </div>
          </FadeIn>
          <FadeIn>
            <div>
              <h2 className="font-display font-bold text-cream text-3xl uppercase tracking-wide mb-1">Automobil B</h2>
              <div className="w-10 h-0.5 bg-red mb-8" />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {conditioning.carB.map((item) => <PriceCard key={item.label + item.duration} item={item} />)}
              </div>
            </div>
          </FadeIn>
          <FadeIn>
            <div>
              <h2 className="font-display font-bold text-cream text-3xl uppercase tracking-wide mb-1">Auto s přívěsem B+E</h2>
              <div className="w-10 h-0.5 bg-red mb-8" />
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {conditioning.carBE.map((item) => <PriceCard key={item.label + item.duration} item={item} />)}
              </div>
            </div>
          </FadeIn>
        </div>
            ...
            <FadeIn>
              <div className="space-y-4">
                <div className="bg-graphite border border-border p-8">
                  <h3 className="font-display font-bold text-cream text-xl uppercase tracking-wide mb-3">Před kondiční jízdou</h3>
                  <p className="text-muted text-sm leading-relaxed">{conditioning.note}</p>
                </div>
                <div className="bg-graphite border border-border p-8">
                  <h3 className="font-display font-bold text-cream text-xl uppercase tracking-wide mb-4">Přímý kontakt</h3>
                  <a href={`tel:${site.phoneRaw}`} className="block font-display font-bold text-cream text-xl hover:text-red transition-colors">{site.phone}</a>
                  <a href={`mailto:${site.email}`} className="block text-muted hover:text-cream transition-colors mt-1">{site.email}</a>
                </div>
              </div>
            </FadeIn>
            <FadeIn delay={0.15}>
